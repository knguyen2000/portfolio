...
- baking _all_ knowledge into the model’s weights, or
...
1. A retriever fetches only the _relevant_ pieces of information.
...
This memory lives _outside_ the model, in things like:
...
One interesting exception: Test-Time Training (TTT), a wild idea that makes the model temporarily learn at inference. This approach was first introduced in 2020 and earlier this year [the authors](https://yueatsprograms.github.io/) released their latest advancement.

### Memory in external environment like python REPL

This is one potential approach to compete with RAG. I did try testing it in my portfolio, you can try that mode in [my main page](/). I will make a discussion on this topic in the future.

The high level idea is data live in Python variable, not LLM context to save context. The model, then, writes code to filter/search/aggregate. In the process, small LLM are called for semantic understanding (which is why it is called recursive). The paper proves that RLM excels in dense information aggregation, pairwise reasoning, and multi-hop reasoning with semantic understanding. Again, you can test it in my main page (it would be interesting though as the paper uses SOTA gpt5 while I'm using small gemma model, but I'll let you have the fun exploring it)

As per what I have been experiencing so far, for now it is yet to be as effective as RAG (in terms of cost and latency), but the potential is there.
...
- Runtime: encode query → retrieve candidates from both indices → fuse/rerank → inject top-_K_ chunks into LLM prompt.  
...
- **Embed each document chunk**
  ...
  - Here:
    - `e_i` is the embedding vector representing chunk _i_
...
1. Dense search → top-_N_ candidates
2. BM25 search → top-_M_ candidates
3. Candidate fusion (merge)
4. Optional second-stage reranking
5. Select final top-_K_ chunks for context assembly
...
Now select final top-_K_ chunks to put into LLM prompt.
...
- **Output**: top _N_ search results, each with:
  - title
  - snippet
  - URL  
    already ranked by the search engine

LLM never sees the whole web. It only sees these _N_ candidates.
...
This step functions like a **heuristic reranker** applied to the search engine’s top-_N_ results.
...
- Selection is mostly **retriever-driven**:
  - embedding similarity or BM25 scoring → top-_K_ chunks
...
Retrieve the top-_k_ documents (or chunks) that are most relevant to the user query, using either dense (embedding-based) or sparse (lexical) similarity:
...
- `topk(·)` selects the _k_ highest-scoring documents
...
In this direction, the model reasons _before_ or _during_ retrieval so that retrieval becomes more targeted and less wasteful.
...
- Reasoning can hallucinate _before_ grounding happens
- Harder to debug when reasoning traces are implicit/hidden

**Formal view**

Instead of retrieving evidence directly from the raw query `q`, retrieval is conditioned on an **intermediate reasoning state** `r`. This state captures the model’s understanding of _how_ the question should be answered, not just _what_ the question asks.
...
- `D* = topk(sim(emb(q_r), emb(d)))`  
  ...
  - documents are selected based on how well they match the _reasoned intent_ of the question
...
It provides _explicit premises_ that LLM can use as constraints in a proof-like chain.
...
- It provides _procedural priors_:
...
- **rerank(top-k)** is a strong refinement operator because it performs _fine-grained relevance discrimination_ over candidates already present.
